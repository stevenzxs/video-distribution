...
`输入N视频调度地址`、`输入N网页取流地址` 和 `输入N网页取流通道`。8001（与
HTTP API 同端口）是
视频调度确认地址，必须携带当前大屏名，例如输出 1 调用
`ws://192.168.130.101:8001/?display_wall=%E6%98%BE%E7%A4%BA%E5%99%A81`；
它返回的是 JSON 成功结果，不是视频帧。按 PDF 附录 17.1，真正取码流还是连接
`ws://192.168.130.101:12997/play`，然后发送 `open_header`。8001 调度确认
握手需要 `Sec-WebSocket-Protocol` 携带当前登录 token；12997 播放取流抓包中
没有该子协议头。后端代理会把 `Origin` 固定为 API 服务地址
`http://192.168.130.101:8001`。代理连接失败日志中的 `stage=调度确认/取流`
用于区分是哪一段上游连接失败；旧文档中的 8003 调度端口是历史记录，实测连接
8003 会被对端静默挂起约 15 秒后空响应关闭。
...
页面预览区会显示 WebSocket 取流状态：`未收到码流` 表示 12997 取流连接没有回帧；
...
WebSocket 取流端口 `12997`，应检查服务器取流服务、防火墙或网络路由。
如果 TCP 可达但 `预览取流握手检查` 显示 `handshake_timeout` 或
`handshake_rejected`，说明 `12997/play` 没有按当前请求正常响应浏览器
