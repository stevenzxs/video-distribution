...
真正取码流仍连接 `ws://192.168.130.101:8003` 根地址。文档示例中的 6 段物理 MAC 需要拼成
...
后端代理用 `Origin=http://192.168.130.101:8001` 和 token 子协议先调 8003，
再连接 8003 根地址取码流。
...
日志会输出 `elapsed=...s`，用于判断平台 8003 取流根地址是否只是响应较慢。
...
表示 8003 取流根地址没有响应握手，`handshake_rejected` 表示服务端明确拒绝握手，
