...
3. 打开大屏
4. 读取当前窗口列表，清理覆盖目标输出区域的旧窗口
5. 开窗显示信号源
6. 再次读取窗口列表，确认返回的 `handle` 或目标坐标中存在期望信号源

Web 矩阵控制台在进程内复用 API 登录会话，不在每次 `1v1.`、`1v2.`、
`2v2.` 等矩阵切换后调用 `Logout`。连续测试时，只要 token 仍有效，
后续切换会直接复用当前 token 继续请求设备、大屏、绑定和开窗接口。
...
如果 `OpenWnd` 返回成功但物理显示器没有画面，不能只看接口业务码。
当前矩阵控制台会在开窗前后调用 `GetDisplayWallWnds`：
- 开窗前关闭覆盖目标输出区域的旧窗口，避免同一区域叠多层窗口或旧窗口遮挡。
- 开窗后确认窗口列表里出现目标输出窗口；例如 `1v1.` 应出现
  `rect=(0,0,1920,1080)` 且信号源为输入 1 编码器。
- 如果目标输出区域已经存在同一输入源的正确窗口，则直接复用该窗口，不再先
  `CloseWnd` 再 `OpenWnd`，避免重复点击同一路矩阵命令时触发 SDK `-10`。

如果日志出现 `开窗确认`，说明平台窗口状态已经落地，后续应检查编码器输入信号、
编码器在线/HDMI状态、解码器与显示器 HDMI 连接、显示器输入源选择等硬件链路。
