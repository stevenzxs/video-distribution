...
- **端口**: 8003
- **协议**: WebSocket
- **连接地址**: `ws://<服务器IP>:8003/?display_wall=<大屏名称>`
- **请求头**: `Origin` 使用 API 服务地址，`Sec-WebSocket-Protocol` 使用登录 token
- **返回**:
```json
{
  "result": "success",
...
该连接只用于通知平台当前要预览的大屏，返回 JSON 成功结果，不返回视频帧。

### 4.2 视频显示取流
- **端口**: 12997
- **协议**: WebSocket
- **连接地址**: `ws://<服务器IP>:12997/play`
- **请求头**: `Origin` 使用 API 服务地址；实测该取流握手不携带 `Sec-WebSocket-Protocol`

### 4.3 取流步骤
1. 先连接 8003 视频调度确认地址，确认返回 `result == "success" or result_val == 0`
2. 再连接 `ws://<服务器IP>:12997/play`
3. 在播放连接上发送取流信息头（JSON格式）:
```json
{
  "a": "",
  "a2": "21",  // 信号源名称
...
Web 页面上方预览使用 WebSocket 另走取流链路。实测 8003 地址需要携带当前
大屏名，例如 `ws://192.168.130.101:8003/?display_wall=%E6%98%BE%E7%A4%BA%E5%99%A81`；
它是视频调度确认地址，返回 JSON 成功结果，不返回视频帧。按 PDF 附录 17.1，
真正取码流仍连接 `ws://192.168.130.101:12997/play`。文档示例中的 6 段物理 MAC 需要拼成
`MAC-00-01/v3`；但实测 `GetEncoderList` 也可能直接返回已经包含通道号的 8 段
MAC，例如 `6c-df-fb-01-5e-80-00-01`。这种情况下取流通道应为
`6c-df-fb-01-5e-80-00-01/v3`，不能重复拼接 `-00-01/v3`。
当前矩阵控制台会自动识别，并在日志中输出 `输入N视频调度地址`、
`输入N网页取流地址` 和 `输入N网页取流通道` 便于核对。
8003 调度确认握手需要 `Sec-WebSocket-Protocol` 携带当前登录 token；12997 播放
取流抓包中没有该子协议头。由于浏览器不能手动指定 `Origin`，页面会优先连接本地
`/api/preview/ws?output=N` 代理，由后端代理用
`Origin=http://192.168.130.101:8001` 和 token 子协议先调 8003，再无子协议连接
`12997/play` 取码流。
...
本地代理上游取流 WebSocket 握手最长等待 20 秒，前端连接最长等待 25 秒；握手诊断
日志会输出 `elapsed=...s`，用于判断平台 `12997/play` 取流地址是否只是响应较慢。
如果 `candidate_failed` 发生在 0 帧阶段，后端会同步做一次 `ws_url` TCP 端口检查；
`status=tcp_failed` 表示本机无法连接取流服务 12997，应优先检查取流服务、防火墙
或网络路由。
TCP 可达时还会做一次标准 WebSocket Upgrade 握手检查；`handshake_timeout`
表示 `12997/play` 没有响应握手，`handshake_rejected` 表示服务端明确拒绝握手，
